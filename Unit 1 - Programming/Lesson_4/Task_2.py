...
#А также написать метод, который принимает список именованных аргументов с новыми значениями и вызывает сеттеры для них.
#Например, если он принимает мотор и коробку передач, то заменить имеющийся мотор на новый и коробку передач тоже.


class Auto:
    def __init__(self, color="red", model="BMW", speed=200, wheels=[17, 17, 19, 19], wipers=['left', 'broken'], seats=['d', 'p', 'p', 'p', 'p'], motor="V8", win=0, transmission="automatic"):
        self.color=color
        self.model=model
        self.speed=speed
        self.wheels=wheels
